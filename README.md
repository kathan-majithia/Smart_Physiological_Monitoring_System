...
\* Feeds the HRV features and the rolling average SpO2 into a pre-trained Scikit-Learn pipeline (`scaler.pkl` \& `stress\_model.pkl`) to generate a diagnosis and confidence score.
...
\# (e.g., 'model\_output/stress\_model.pkl' and 'model\_output/scaler.pkl')
...
3\. Update the Wi-Fi credentials and `SERVER\_URL` in `main.py` to match your local network and Flask IP.
...
The system does not rely on the MAX30102 for heart rate calculation during ML inference due to optical motion artifacts. Instead, it extracts the mean\_hr directly from the electrical AD8232 signal for medical-grade precision.
